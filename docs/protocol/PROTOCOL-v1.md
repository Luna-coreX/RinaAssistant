...
**Создать можно не только голосом.** `reminders.create` и конструктор команд появились в `4.0-F04`, когда обе страницы оказались пусты: список был, а завести первую запись из окна было нечем. Голосом напоминание заводится разбором фразы, и казалось, что окну хватит того же пути; не хватает — человек у экрана выбирает время в поле, а не проговаривает его словами, и составлять русскую фразу ради обратного разбора значит проверять разбор вместо намерения. Время поэтому приходит меткой: часовой пояс у оболочки и ядра один, они на одной машине.

**`commands.builtin` — по той же причине.** Перечень встроенного — это **фразы, которые говорят Рине**, то есть часть её словаря, а не подписи интерфейса (`4.0-F08`). Оболочка, знающая их наизусть, показала бы то, чего ядро уже не понимает, и человек сказал бы это вслух впустую.

**`setup.state` — вопрос о сеансе, а не настройка.** Ключ `first_run` помечен секретным и наружу через `settings.get` не отдаётся, и правильно: это состояние хранилища, а не то, что человек правит. «Настроен ли уже этот человек» — вопрос той же природы, что «на связи ли ядро», и у него своя дверь, а не дырка в чужой.
...
**Возможность `system` объявляет оболочка**, и объявляет честно: у оболочки без Windows её не будет, и ядро узнает об этом из рукопожатия, а не из молчания в ответ. Решение — [ADR 0009](../adr/0009-system-layer.md): **машину трогает оболочка, решает, что с ней сделать, ядро**.
...
**Ответ оболочки — факт, а не предложение.** `system.do` отвечает `ok` и подробностью; «Прибавила громкость» составляет ядро, потому что это реплика Рины ([ADR 0007](../adr/0007-localisation.md)).
...
**Перебивание — не закрытие потока** (`4.0b-E12`). Событие `speech.stop`
означает «оборви речь и выброси недоигранное»: поток остаётся открытым,
потому что следующая реплика пойдёт по нему же. Закрытие — это конец
разговора с устройством, а перебивание — реплика в этом разговоре.

```json
{"type": "event", "method": "speech.stop", "payload": {}}
```

Обратного подтверждения нет: перебивание либо случилось у получателя,
либо играть уже было нечего, и третьего исхода не бывает.
...
Модель распознавания — сто сорок мегабайт для Whisper, около сорока пяти для Vosk. На небыстрой линии это минуты, а минуты без единой строки на экране неотличимы от зависшей программы. Поэтому ход объявляется событиями, а не прячется внутри вызова, который собирался распознать фразу.

`share` даётся вместе с `done` и `total`, чтобы оболочке не пришлось делить и решать, что показывать при неизвестном размере; при неизвестном он ноль. `name` — чтобы можно было сказать, **что** качается, а не «идёт загрузка».

**Своих событий у скачивания нет, и это решение, а не упущение.** Ход идёт обычным `task.progress` (§9), отмена — обычным `task.cancel`. Скачивание — такая же длящаяся работа, как ответ модели или разбор архива: доля в `fraction`, пояснение в `note` — «скачиваю модель Whisper, 12 из 140 МБ». Три события `model.*` были написаны и убраны до первого применения: они говорили бы то же самое вторым способом, а два способа сказать одно — это ровно тот случай, когда один из них однажды забудут поддержать.
...
**Предупреждения помнит сама запись, а не планировщик.** Планировщик перезапускается вместе с программой, и человек, оставивший Рину на ночь, слышал бы «через три часа» каждое утро заново. Список закрыт: секунды берутся из `AHEAD`, и значение не из него ничего не означает.

**У `on` и `fire_at` ровно один хозяин.** Напоминание срабатывает либо по часам, либо по поводу; у привязанного к поводу часов нет вовсе, и `0` здесь означает «не по часам», а не «в 1970 году». Если бы оно значило время, планировщик счёл бы такую запись просроченной на полвека и разбудил бы человека через секунду после того, как тот её завёл.

**Повод сравнивается по пути запуска, а не по имени окна.** Заголовок окна человек меняет сам, открыв в редакторе чужой файл; имя программы он говорит как придётся. Путь разрешается один раз, в момент заведения, тем же индексом, что и запуск, — дальше сравнение точное и без догадок. Регистр не учитывается: Windows его не различает.
